...
## Over-dispersed counts {#sec-Over-dispersed-counts}
...
If you'd like to see how to make a similar plot in terms of $\alpha$ and $\beta$, see [Chapter 6](https://solomon.quarto.pub/dbda2/06.html) of my [-@kurzDoingBayesianDataAnalysis2026] [ebook](https://solomon.quarto.pub/dbda2/) wherein I translated Kruschke's text into **tidyverse** and **brms** code.
...
`inv_gamma(0.4, 0.3)` is the **brms** default for the `shape` parameter in this model.^[Back in the day, **brms** used to default to $\operatorname{InvGamma}(0.01, 0.01)$ in this context. Times have changed. For the details, see [this issue](https://github.com/paul-buerkner/brms/issues/1614).] Sadly, the base-**R** **stats** package does not have convince functions for the inverse gamma distribution, but we do have access to a `dinvgamma()` function from the **invgamma** package.
...
Do you remember the monk data from back in @sec-Exposure-and-the-offset? Here we simulate some more. This time we'll work in a little alcohol.
...
For more on how to fit this model with **rstan**, go to my companion book [here](https://solomon.quarto.pub/sr2rstan/12.html#zero-inflated-outcomes).

## Ordered categorical outcomes {#sec-Ordered-categorical-outcomes}
...
### Adding predictor variables {#sec-Adding-predictor-variables}
