...
where $s$ indexes one of our five latent strategies and $\Pr(j \mid s)$ is the probability of one of the three color choices given a child is using the $s$th strategy. The $\sum_{s = 1}^5 p_s$ portion conveys these five probabilities are treated as a simplex, which means they will sum to one. We give these probabilities a Dirichlet prior, which we learned about back in @sec-Ordered-categorical-predictors. Here's what that prior looks like.
...
### Coding the statistical model {#sec-Coding-the-statistical-model}
...
However, we do need to discuss McElreath's `Boxes_model` code. The recommended syntax for Stan has evolved over time, and McElreath's `Boxes_model` uses some older conventions that are no longer supported. If you try fitting it now (01-2026), you'll probably get a warning message. Based on the discussion in [this GitHub issue](https://github.com/rmcelreath/rethinking/issues/414), I recommend you use this code instead. For clarity, I'll name it `boxes_model`.^[I'm borrowing heavily from Michael S. Truong's `Boxed_model` [code](https://github.com/rmcelreath/rethinking/issues/414#issuecomment-1828422433), here.]
...
As an alternative, this might be a good time to revisit the `tidybayes::stat_ccdfinterval()` approach (see @sec-Adding-predictor-variables), which will depict those posteriors with a bar plot where the ends of the bars depict our uncertainty in terms of cumulative density curves.
...
The next step is to write a script that will tell **brms** how to tell Stan how to fit a Lotka-Volterra model. In his blog, Gesmann called this `LotkaVolterra`. Our script to follow is a very minor adjustment of his.^[You might also note this script is an update from earlier versions of the ebook. Stan has updated some of its recommended syntax, and it has also discarded the old `integrate_ode_rk45()` function for `ode_rk45()`.]
...
Did you notice how the fourth line in the output read 'Number of observations: 20?' That's because we had that one missing value for `Hare_1`. One quick and dirty hack might be to use the missing data syntax we learned from @sec-Missing-Data-and-Other-Opportunities. Here's how that might look.
