# Models With Memory {#sec-Models-With-Memory}
...
Now fit this simple aggregated binomial model much like we practiced in @sec-Chimpanzees-again-condensed.
...
b13.1 <- add_criterion(b13.1, criterion = "loo")
b13.2 <- add_criterion(b13.2, criterion = "loo")
```
...
For more on how to fit this model with **rstan**, go to my companion book [here](https://solomon.quarto.pub/sr2rstan/13.html#varying-effects-and-the-underfittingoverfitting-trade-off).
...
The initial multilevel update from model `b11.4` from @sec-Prosocial-chimpanzees follows the statistical formula
...
If you jump ahead to @sec-Non-centered-chimpanzees, you'll see this is just re-write of the formula on the top of page 424. For now, let's load the data.
...
#### Bonus: Let's use `fitted()` this time {#sec-Lets-use-fitted-this-time}
...
### Post-stratification {#sec-Post-stratification}
...
For a more advanced application, check out the paper by [Kolczynska](https://martakolczynska.com/), Bürkner, [Kennedy](https://jazzystats.com/about/), and Vehtari [-@kolczynskaTrustStateInstitutions2020], which combines MRP with a model with ordinal outcomes (recall @sec-Ordered-categorical-outcomes). Their supplemental material, which includes their **R** code, lives at [https://osf.io/dz4y7/](https://osf.io/dz4y7/). With all this good stuff, it seems we have an embarrassment of riches when it comes to **brms** and MRP! To wrap this section up, we'll give Monica Alexander the last words:
