# Ulysses' Compass {#sec-Ulysses-Compass}
...
### More parameters (almost) always improve fit {#sec-More-parameters-almost-always}
...
If you really what the axes scaled in the original metrics of the variables rather than their standardized form, you can use the re-scaling techniques from back in @sec-Converting-back-to-natural-scale.
...
# b7.1_demo_fit <- brain_loo_lines(b7.1, row = 1)
# save(b7.1_demo_fit, file = "fits/b7.1_demo_fit.rda")

load("fits/b7.1_demo_fit.rda")
...
# save(b7.1_fits, file = "fits/b07.01_fits.rda")
# save(b7.4_fits, file = "fits/b07.04_fits.rda")

load("fits/b07.01_fits.rda")
load("fits/b07.04_fits.rda")
```
...
> An important practical consequence of this asymmetry, in a model fitting context, is that if we use a distribution with high entropy to approximate an unknown true distribution of events, we will reduce the distance to the truth and therefore the error. This fact will help us build generalized linear models, later on in @sec-Big-Entropy-and-the-GLM. (p. 209)
...
#### Overthinking: Computing the lppd {#sec-Computing-the-lppd}
...
# 28.09169 mins to
# save(list = c("dev_100", "dev_20", "kseq", "n_sim", "n_cores"), file = "sims/07.sim_1.rda")
...
# make_sim(n = 20, k = 1, b_sigma = 0.5)

# Not currently working properly. Gets stuck on this setting
crossing(
  n       = 20,
  k       = 2,
  b_sigma = 0.5) |>
  mutate(sim = pmap(.l = list(n, k, b_sigma), .f = make_sim)) |> 
  unnest(sim)
...
# save(list = c("s20_0.5", "s20_100", "s100_0.5", "s100_100"), file = "/Users/solomonkurz/Dropbox/Recoding Statistical Rethinking/2nd ed/sims/07.sim_3.rda")

# load(file = "sims/07.sim_3.rda")
...
A variety of approaches to model averaging are available with **brms** and I covered them in my [-@kurzStatisticalRethinking2026] ebook translation of the first edition of McElreath's text.
...
Refit the divorce models from @sec-Spurious-associations.
...
As $\nu \rightarrow \infty$, the standard deviation approaches the scale, $\sigma$. We'll have more practice with robust Student's $t$ regression later on. In the mean time, you might check out chapters [16](https://solomon.quarto.pub/dbda2/16.html#sec-Outliers-and-robust-estimation-The-t-distribution), [17](https://solomon.quarto.pub/dbda2/17.html#sec-Robust-linear-regression), [18](https://solomon.quarto.pub/dbda2/18.html#sec-The-model-and-implementation), [19](https://solomon.quarto.pub/dbda2/19.html#the-two-group-case), and [20](https://solomon.quarto.pub/dbda2/20.html#heterogeneous-variances-and-robustness-against-outliers) from my [-@kurzDoingBayesianDataAnalysis2026] ebook translation of Kruschke's text. I've also blogged on using the robust Student's $t$ for [regression](https://solomonkurz.netlify.app/blog/2019-02-02-robust-linear-regression-with-student-s-t-distribution/) and [correlations](https://solomonkurz.netlify.app/blog/2019-02-10-bayesian-robust-correlations-with-brms-and-why-you-should-love-student-s-t/). @gelmanRegressionOtherStories2020 covered robust Student's $t$ regression in Section 15.6, too.
...
At the beginning of the chapter (pp. 193--201), McElreath introduced $R^2$ as a popular way to assess the variance explained in a model. He pooh-poohed it because of its tendency to overfit. It's also limited in that it doesn't generalize well outside of the single-level Gaussian framework, which will be a big deal for us starting in @sec-Big-Entropy-and-the-GLM. However, if you should find yourself in a situation where $R^2$ suits your purposes, the **brms** `bayes_R2()` function might be of use. Simply feeding a model `brm()` fit object into `bayes_R2()` will return the posterior mean, $\textit{SD}$, and 95% intervals. For example:
