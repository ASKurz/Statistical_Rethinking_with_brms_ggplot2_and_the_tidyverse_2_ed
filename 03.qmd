...
tibble(pr_positive_vampire = 0.95,
...
We used the `slice_min()` to compute, and then save the exact minimum value as `min_loss[1]`, which is `r min_loss[1]`. Within sampling error, this is the posterior median as depicted by our `samples`.
...
We'll learn more about the beta distribution in @sec-Monsters-and-Mixtures. But for now, here's the posterior summary for `b_Intercept`, the probability of a "w".
