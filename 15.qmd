...
d <- tibble(seed = 1:n_sim) |> 
  mutate(burnt = map(.x = seed, .f = sim_pancake)) |> 
  unnest(burnt) |> 
...
We dropped those values when we fit the models back in @sec-Masked-relationship. To get a sense of whether this was a bad idea, let's consider the model with a DAG. Ignoring the missing data, we have this.
...
## Categorical errors and discrete absences {#sec-Categorical-errors-and-discrete-absences}
...
where the last two lines spell out our priors. As we learned in @sec-Binomial-regression, the $\operatorname{Normal}(0, 1.5)$ prior in the log-odds space is just about flat on the probability space. If you wanted to be more conservative, consider something like $\operatorname{Normal}(0, 1)$. Here's how to fit the model with **brms**.
...
### Fit the Bayesian muiltilevel alternative {#sec-Fit-the-Bayesian-muiltilevel-alternative}
...
This provides some information, which we wouldn't get from a meta-analysis. However, the plot is a little deceiving. First, it doesn't provide an expression of the uncertainty on the effects. Second, it doesn't give full justice to the correlation between the two effects, $\rho_{\alpha \beta}$. If you look back up at the `print()` output from @sec-Fit-the-Bayesian-muiltilevel-alternative, you'll see that correlation was about `r round(VarCorr(b15.11)$loc$cor[2, 1, "Intercept"], digits = 2)`. We can take some cues from @sec-Social-relations-as and express the bivariate posteriors as ellipses, rather than simple mean-level points.
