...
If you really what the axes scaled in the original metrics of the variables rather than their standardized form, you can use the re-scaling techniques from back in the end of Chapter 4 (p. 114).
...
"It would be possible to use **ordinary least squares** (OLS) to get posterior distributions for these brain size models" (p. 196, **emphasis** in the original). So see an example of an alternative OLS-based workflow for this polynomial models and their figure, check out my [-@kurzStatisticalRethinkingBrms2020] [ebook translation](https://bookdown.org/content/3890/overfitting-regularization-and-information-criteria.html#more-parameters-always-improve-fit.) of Section 6.1.1 from McElreath's [-@mcelreathStatisticalRethinkingBayesian2015] first edition.
...
In his **Overthinking: Dropping rows** box, McElreath encouraged us to take a look at the `brain_loo_plot()` function to get a sense of how he made his Figure 7.4. Here it is.
...
Our systems in this section are binary (e.g., $q = \{ q_i, q_2 \}$). Thus if you know $q_1 = .3$ you know of a necessity $q_2 = 1 - q_1$. Therefore we can code the tibble for the next example of when $p = q$ like this.
...
## Intercept     0.53      0.08     0.36     0.69 1.00     2749     2222
## mass_std      0.17      0.09    -0.03     0.35 1.00     2157     2002
## 
## Family Specific Parameters: 
##       Estimate Est.Error l-95% CI u-95% CI Rhat Bulk_ESS Tail_ESS
## sigma     0.22      0.08     0.12     0.42 1.00     1745     1891
## 
...
```
##  num [1:4000, 1:7] 0.241 0.678 0.547 0.556 -0.596 ...
...
Happily, it returned the same results as McElreath's **R** code 7.13. Our **brms** analogue to the output from `sim(quap_fit, ll = TRUE, n = 1e4)` is to inset a `brm()` model fit into the `log_lik()` function. Although the way those two functions compute their output differes, the format is largely the same. With that in mind, here's a mild reworking of our `lppd_r()` funciton designed to work with **brms** fits.
...
```
## [1]  0.5229569  0.5596560  0.4707154  0.5359276  0.3892479  0.3557682 -0.7326598
```
...
To dive even deeper into the horseshoe prior, check out Michael Betancourt's [-@betancourtBayesSparse2018] tutorial, [*Bayes sparse regression*](https://betanalpha.github.io/assets/case_studies/bayes_sparse_regression.html#35_the_horseshoe).
...
where $y$ is the data, $\Theta$ is the posterior distribution, and $\text{lppd}$ is the log-posterior-predictive-density from before. The penalty term is also referred to at the effective number of parameters, $p_\text{WAIC}$. There are a few ways to compute the WAIC with **brms**, including with the `waic()` funtion.
...
## Population-Level Effects: 
##           Estimate Est.Error l-95% CI u-95% CI Rhat Bulk_ESS Tail_ESS
## Intercept   -17.47      6.04   -29.16    -5.55 1.00     3325     2531
## speed         3.92      0.37     3.21     4.64 1.00     3395     2547
## 
## Family Specific Parameters: 
##       Estimate Est.Error l-95% CI u-95% CI Rhat Bulk_ESS Tail_ESS
## sigma    13.78      1.19    11.64    16.36 1.00     3778     2902
## 
## Samples were drawn using sampling(NUTS). For each parameter, Bulk_ESS
## and Tail_ESS are effective sample size measures, and Rhat is the potential
## scale reduction factor on split chains (at convergence, Rhat = 1).
...
Now use the `brms::log_lik()` function to return the loglikelihood for each observation $i$ at each posterior draw $s$, where $S = 4,000$.
...
## [6,] -3.740294 0.02133788 7.480587
```
...
## b6.8  -20.6       4.9  -201.2       5.4          2.4    0.3     402.4   10.8 
...
## b6.8  41.12781  9.863271
## b6.6  44.58404 11.615248
```
...
```
##  'compare.loo' num [1:3, 1:8] 0 -20.56 -22.29 0 4.93 ...
##  - attr(*, "dimnames")=List of 2
...
## b6.7    0.0       0.0  -180.7      6.7         3.4    0.5    361.3   13.5  
## b6.8  -20.6       4.9  -201.2      5.4         2.4    0.3    402.5   10.8  
## b6.6  -22.3       5.8  -203.0      5.7         1.5    0.2    405.9   11.4
```
...
With our **brms** paradigm, we won't get a comparrison plot by inserting `loo_compare(b6.6, b6.7, b6.8, criterion = "waic")` within `plot()`. But with a little `[]` subsetting and light wrangling, we can convert the contents of our `w` object to a format suitable for plotting the WAIC estimates with **ggplot2**.
...
A variety of approaches to model averaging are avaliable with **brms** and I covered them in my [-@kurzStatisticalRethinkingBrms2020] ebook translation of the first edition of McElreath's text.
...
Refit the divorce models from Chapter 5.
...
```r
b5.1 <- add_criterion(b5.1, "loo", file = "fits/b05.01.rds")
b5.2 <- add_criterion(b5.2, "loo", file = "fits/b05.02.rds")
b5.3 <- add_criterion(b5.3, "loo", file = "fits/b05.03.rds")
```
...
## b5.2  -6.7       4.6   -69.7      4.9         3.0   0.9    139.4   9.9
```
...
0.61 is a little high, but not high enough to cause `loo()` to return a warning message. Once a Pareto $k$ value crosses the 0.7 threshold, though, the **loo** package will bark. Before we make our version of Figure 7.10, we’ll want to compute the WAIC for `b5.3`. That will give us access to the $p_\text{WAIC}$.


```r
b5.3 <- add_criterion(b5.3, "waic", file = "fits/b05.03.rds")
```
...
In the text, McElreath reported the effective number of parameters for `b5.3` was nearly 6. We can look at this with the `waic()` function.
...
## Population-Level Effects: 
##           Estimate Est.Error l-95% CI u-95% CI Rhat Bulk_ESS Tail_ESS
## Intercept     0.02      0.10    -0.17     0.22 1.00     3866     3165
## m             0.05      0.20    -0.31     0.46 1.00     3130     2790
## a            -0.70      0.14    -0.98    -0.42 1.00     2768     2793
## 
## Family Specific Parameters: 
##       Estimate Est.Error l-95% CI u-95% CI Rhat Bulk_ESS Tail_ESS
## sigma     0.58      0.09     0.43     0.77 1.00     3952     2762
## 
## Samples were drawn using sampling(NUTS). For each parameter, Bulk_ESS
## and Tail_ESS are effective sample size measures, and Rhat is the potential
## scale reduction factor on split chains (at convergence, Rhat = 1).
...
```r
b5.3t <- add_criterion(b5.3t, c("loo", "waic"), file = "fits/b05.03t.rds")
```

Now remake the plot from Figure 7.10, this time based on the $t$ model.
...
## b5.3t  -2.6       2.9   -66.3       5.8          6.0    1.0     132.7  11.6
```
...
## b5.3t  -2.5       3.0   -66.3      5.8         6.0   1.0    132.6  11.6
```
...
bind_rows(posterior_samples(b5.3),
  ...
  stat_pointintervalh(.width = .95, size = 1) +
  scale_color_manual(values = c(carto_pal(7, "BurgYl")[6], "black")) +
  ...
  facet_wrap(~name, scales = "free_x", ncol = 1, labeller = label_parsed)
```
...
r2 <-
  ...
  labs(title = expression(italic(R)^2~distributions),
       subtitle = "Going from left to right, these are\nfor models b5.2 and b5.1.")
```

<img src="07_files/figure-gfm/unnamed-chunk-106-1.png" width="336" style="display: block; margin: auto;" />
...
##  [1] loo_2.2.0            brms_2.13.0          Rcpp_1.0.4.6         patchwork_1.0.0     
##  [5] tidybayes_2.1.1      rethinking_2.01      dagitty_0.2-2        rstan_2.19.3        
##  [9] StanHeaders_2.21.0-1 ggrepel_0.8.2        rcartocolor_2.0.0    forcats_0.5.0       
## [13] stringr_1.4.0        dplyr_0.8.5          purrr_0.3.4          readr_1.3.1         
## [17] tidyr_1.0.2          tibble_3.0.1         ggplot2_3.3.1        tidyverse_1.3.0     
## 
...
##  [25] haven_2.2.0          xfun_0.13            callr_3.4.3          crayon_1.3.4        
##  [29] jsonlite_1.6.1       survival_3.1-12      zoo_1.8-7            glue_1.4.1          
##  [33] gtable_0.3.0         emmeans_1.4.5        V8_3.0.2             pkgbuild_1.0.8      
##  [37] shape_1.4.4          abind_1.4-5          scales_1.1.1         mvtnorm_1.1-0       
##  [41] DBI_1.1.0            miniUI_0.1.1.1       viridisLite_0.3.0    xtable_1.8-4        
##  [45] stats4_3.6.3         DT_0.13              htmlwidgets_1.5.1    httr_1.4.1          
##  [49] threejs_0.3.3        arrayhelpers_1.1-0   ellipsis_0.3.1       pkgconfig_2.0.3     
##  [53] farver_2.0.3         dbplyr_1.4.2         utf8_1.1.4           tidyselect_1.0.0    
##  [57] labeling_0.3         rlang_0.4.6          reshape2_1.4.4       later_1.0.0         
##  [61] munsell_0.5.0        cellranger_1.1.0     tools_3.6.3          cli_2.0.2           
##  [65] generics_0.0.2       broom_0.5.5          ggridges_0.5.2       evaluate_0.14       
##  [69] fastmap_1.0.1        yaml_2.2.1           processx_3.4.2       knitr_1.28          
##  [73] fs_1.4.1             nlme_3.1-144         mime_0.9             xml2_1.3.1          
##  [77] compiler_3.6.3       bayesplot_1.7.1      shinythemes_1.1.2    rstudioapi_0.11     
##  [81] curl_4.3             reprex_0.3.0         stringi_1.4.6        ps_1.3.3            
##  [85] Brobdingnag_1.2-6    lattice_0.20-38      Matrix_1.2-18        markdown_1.1        
##  [89] shinyjs_1.1          vctrs_0.3.1          pillar_1.4.4         lifecycle_0.2.0     
##  [93] bridgesampling_1.0-0 estimability_1.3     httpuv_1.5.2         R6_2.4.1            
##  [97] bookdown_0.18        promises_1.1.0       gridExtra_2.3        codetools_0.2-16    
...
## [113] shiny_1.4.0.2        lubridate_1.7.8      base64enc_0.1-3      dygraphs_1.1.1.6
```
