...
where $y$ is some count variable, $n$ is the number of trials, and $p$ it the probability a given trial was a 1, which is sometimes termed a *success*. When $n = 1$, then $y$ is a vector of 0's and 1's. Presuming the logit link,^[Though McElreath didn't cover it, here, it's also fine to fit binomial models using the probit link. @gelmanRegressionOtherStories2020 covered probit regression in Section 15.4. With **brms**, it's simply a matter of setting `family = binomial(link = "probit")` within `brm()`.] which we just covered in @sec-Linking-linear-models-to-distributions, models of this type are commonly termed logistic regression. When $n > 1$, and still presuming the logit link, we might call our model an aggregated logistic regression model, or more generally an aggregated binomial regression model.
...
It would be conventional to include these two variables and their interaction using dummy variables. We're going to follow McElreath and use an index variable approach, instead. If you'd like to see what this would look like using the dummy variable approach, check out my [-@kurzStatisticalRethinking2026] [translation of the corresponding section](https://solomon.quarto.pub/sr/10.html#sec-Prosocial-chimpanzees) from McElreath's first [-@mcelreathStatisticalRethinkingBayesian2015] edition. For now, make the index, which we'll be saving as a factor.
...
For retrieving log-probability summaries, our approach with **brms** is a little different than the one you might take with McElreath's **rethinking**. Rather than adding a `log_lik=TRUE` argument within `rethinking::ulam()`, we just use the `log_lik()` function after fitting a **brms** model. You may recall we already practiced this way back in @sec-Computing-the-lppd. Here's a quick example of what that looks like for `b11.5`.

```{r}
log_lik(b11.5) |> str()
```

#### Bonus: A truly flat prior {#sec-A-truly-flat-prior}

We just used the `normal(0, 1.5)` prior for the intercept $\alpha$ as a very weakly regularizing, almost flat prior. I think this prior is a great default for intercepts in logistic regression models. However, if you want a truly flat prior for the intercept in these cases, you can use `prior(logistic(0, 1))`. As [Matthijs Hollanders](https://github.com/mhollanders) shared on Bluesky ([here](https://bsky.app/profile/matthollanders.bsky.social/post/3ma7ylwguvc2n)), "logistic(0, 1) is literally uniform on [0, 1]." He, of course, is right.
...
### Aggregated binomial: Chimpanzees again, condensed {#sec-Chimpanzees-again-condensed}
...
where departments are indexed by $k$. To fit a model including two index variables like this in **brms**, we'll need to switch back to the non-linear syntax. Though if you'd like to see an analogous approach using conventional **brms** syntax, check out model `b10.9` in [Section 10.1.3](https://solomon.quarto.pub/sr/10.html#aggregated-binomial-graduate-school-admissions) of my translation of McElreath's first edition. 
...
#### Overthinking: Modeling tool innovation {#sec-Modeling-tool-innovation}
...
# Informative, but not as clear as the dot plot

tibble(b11.10 = b11.10$criteria$loo$diagnostics$pareto_k,
       b11.11 = b11.11$criteria$loo$diagnostics$pareto_k) |> 
  
  ggplot(aes(x = b11.10, y = b11.11)) +
  geom_abline(color = alpha("white", 1/5)) +
  geom_hline(yintercept = c(0.5, 0.7, 1), linetype = 3, color = wes_palette("Moonrise2")[2]) +
  geom_vline(xintercept = c(0.5, 0.7, 1), linetype = 3, color = wes_palette("Moonrise2")[2]) +
  geom_point(color = wes_palette("Moonrise2")[1]) +
  scale_x_continuous(breaks = c(0, 0.5, 0.7, 1)) +
  scale_y_continuous(breaks = c(0, 0.5, 0.7, 1)) +
  coord_equal(xlim = c(0, 1),
              ylim = c(0, 1)) +
  ggtitle(expression(Pareto~italic(k)~comparison))
```
...
### Example: Exposure and the offset {#sec-Exposure-and-the-offset}
...
For more practice fitting multinomial models with **brms**, check out [Chapter 22](https://solomon.quarto.pub/dbda2/22.html) of my [-@kurzDoingBayesianDataAnalysis2026] translation of Kruschke's [-@kruschkeDoingBayesianData2015] text.
...
McElreath fit his survival model using the exponential likelihood. We briefly met the exponential likelihood in @sec-Big-Entropy-and-the-GLM. As McElreath wrote:
