...
>
> In neither application do [we] have to intuit the consequences of measurement errors nor the implications of missing values in order to design the models. All [we] have to do is state [the] information about the error or about the variables with missing values. Logic does the rest. [@mcelreathStatisticalRethinkingBayesian2020, p. 490, **emphasis** in the original]
...
In anticipation of **R** code 15.3, wrangle the data a little.
...
The **brms** package currently [^4] offers `theme_black()`, which changes the default **ggplot2** theme to a black background with white lines, text, and so forth. The origins of this version of the code are from [this post](https://jonlefcheck.net/2013/03/11/black-theme-for-ggplot2-2/) on Jon Lefcheck's website.
...
[^4]: At the time of this writing, `brms::theme_black()` is depreciated [see @brms2020RM]. If you try using it, you'll get a warming message suggesting you use the [**ggdark** package](https://CRAN.R-project.org/package=ggdark) [@R-ggdark] for dark themes. To my eye, **ggdark** looks like a nice package.
...
Use **ggdag** to help make a DAG of our data.
...
Our rows `Yl[1]` through `Yl[50]` correspond to what **rethinking** named `D_true[1]` through `D_true[50]`.
...
posterior_samples(b15.1) %>% 
  ...
  stat_lineribbon(aes(y = fitted),
                  .width = .95, size = 1, color = "white", fill = "grey25") +
  geom_point(data = data_error,
             aes(y = D_obs),
             color = color) +
  geom_point(data = data_error,
             aes(y = Estimate),
...
```{r}
nd <- 
  tibble(A = seq(from = -3.5, to = 3.5, length.out = 50),
         M = 0)
