---
title: "Ch. 7 Overfitting, Regularization, and Information Criteria"
...
* underfitting, "which leads to poor prediction by learning too *little* from the data" (p. 188, *emphasis* added) 

#### Rethinking stargazing.

"Whatever you think about null hypothesis significance testing in general, using it to select among structurally different models is a mistake--$p$-values are not designed to help you navigate between underfitting and overfitting" (p. 189). As you’ll may have noticed by now, we will generally eschew $p$-values and NHST in this project.
...
> The proper thing to do with those priors is to simulate the prior predictive distribution. But let’s go with the priors above, for the sake of the lesson. The first lesson of these priors is what they imply. The prior for $\alpha$ is just centered on the mean brain volume (rescaled) in the data. So it says that the average species with an average body mass has a brain volume with an 89% credible interval from about −1 to 2. That is ridiculously wide and includes impossible (negative) values. The prior for $\beta$ is very flat and centered on zero. It allows for absurdly large positive and negative relationships. The second lesson of these priors is that they allow for absurd inferences, especially as the model gets more complex. (p. 192)

Warning: These priors are a mess with brms. This will become very clear when we plot, below. If you want to reign in the widths of the posteriors, try reducing the $\beta$ prior to something more like $\text{Normal} (0, 2)$.
...
```{r b7.1, cache = T, message = F, warning = F}
b7.1 <- 
...
```{r b7.2, cache = T, message = F, warning = F}
b7.2 <- 
  update(b7.1,
...
```{r b7.3_through_b7.5, cache = T, message = F, warning = F}
b7.3 <- 
  update(b7.1,
...
McElreath's trick fixing `sigma = 0.001` works in brms, too. But note that in `brm()`, we do so within the `bf()` function.

```{r b7.6, cache = T, message = F, warning = F}
b7.6 <- 
  brm(data = d, family = gaussian,
