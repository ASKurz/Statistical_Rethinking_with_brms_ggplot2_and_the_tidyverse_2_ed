...
Although our `d` tibble has only three columns, the `X` column is a matrix column into which we've smuggled ten more columns. Here's how we might access them more directly.
...
See? There's an $100 \times 10$ data matrix in there. Here's how to fit the full model with **brms** where we use the compact matrix-column syntax in the `formula` argument.
