...
In @sec-More-than-one-type-of-cluster we saw that data can be considered **cross-classified** if they have multiple grouping factors. We used the `chipanzees` data in that section and we only considered cross-classification by single intercepts. Turns out cross-classified models can be extended further. Let's load and wrangle those data.
...
For more practice with models of this kind, check out my blog post, [*Multilevel models and the index-variable approach*](https://solomonkurz.netlify.app/blog/2020-12-09-multilevel-models-and-the-index-variable-approach/).
...
"The instrumental variable model is often discussed with an estimation procedure known as **two-stage least squares** (2SLS)" (p. 460, **emphasis** in the original). For a nice introduction to instrumental variables via 2SLS, see this [practical introduction](https://evalf20.classes.andrewheiss.com/example/iv/), and also the [slides and video-lecture files](https://evalf20.classes.andrewheiss.com/content/11-content/), from the great [Andrew Heiss](https://www.andrewheiss.com/). For the clinical researchers in the room, [Kristoffer Magnusson](https://rpsychologist.com/about) has a nice **brms**-based blog post on the topic called [*Confounded dose-response effects of treatment adherence: fitting Bayesian instrumental variable models using brms*](https://rpsychologist.com/adherence-analysis-IV-brms).
...
>
> In addition to the backdoor criterion you met in @sec-The-Haunted-DAG, there is something called the **front-door criterion**. (p. 460, **emphasis** in the original)
...
# To avoid issues when rendering to HTML
# precis_m14.7_1 <- precis(m14.7) |>
#   data.frame()
# precis_m14.7_3 <- precis(m14.7, depth = 3, pars = c("Rho_gr", "sigma_gr")) |>
#   data.frame()
# post <- extract.samples(m14.7)

# save(list = c("kl_data", "m14.7", "precis_m14.7_1", "precis_m14.7_3", "post"), file = "fits/m14.7.rda")
...
```{r no_eval_precis_m14.7_1}
#| eval: false

precis(m14.7)
```

```{r no_echo_precis_m14.7_1}
#| echo: false
...
```{r no_eval_precis_m14.7_3}
#| eval: false

precis(m14.7, depth = 3, pars = c("Rho_gr", "sigma_gr"))
```

```{r no_echo_precis_m14.7_3}
#| echo: false
...
```{r no_eval_extract.samples_m14.7}
#| eval: false
...
Our Gaussian process model is an extension of the non-linear model from @sec-Modeling-tool-innovation, `b11.11`. Thus our model here will also use the non-linear syntax. Here's how we might use **brms** to fit our amended non-centered version of McElreath's `m14.8`.
...
# To avoid issues when rendering to HTML
# precis_m14.11 <- precis(m14.11) |> 
#   data.frame()
# post <- extract.samples(m14.11)

# save(list = c("m14.11", "precis_m14.11", "post"), file = "fits/m14.11.rda")
...
```{r no_eval_precis_m14.11}
#| eval: false

precis(m14.11)
```

```{r no_echo_precis_m14.11}
#| echo: false
...
You may have noticed my statistical notation differs a bit from McElreath's, here. It follows a blend of sensibilities from McElreath, Williams, and from the notation I used in my [-@kurzAppliedLongitudinalDataAnalysis2026] translation](https://solomon.quarto.pub/alda/) of Singer and Willett's [-@singerAppliedLongitudinalData2003] text, [*Applied longitudinal data analysis: Modeling change and event occurrence*](https://oxford.universitypressscholarship.com/view/10.1093/acprof:oso/9780195152968.001.0001/acprof-9780195152968). I hope it's clear.
...
* My **brms**/**tidyverse** translation of that text, [*Applied Longitudinal Data Analysis in brms and the tidyverse *](https://solomon.quarto.pub/alda/); or
