...
We'll approach the first two in this chapter. Interactions are reserved for @sec-Ulysses-Compass.
...
### Think before you regress {#sec-Think-before-you-regress}
...
##### Bonus: Rethinking residuals {#sec-Bonus-Rethinking-residuals}
...
#### Counterfactual plots {#sec-Counterfactual-plots}
...
## Masked relationship {#sec-Masked-relationship}
...
Here are three. I'm not aware we can facet `dagify()` objects. But we can take cues from @sec-Geocentric-Models to link our three DAGs like McElreath did his. first, we'll recognize the **ggplot2** code will be nearly identical for each DAG. So we can just wrap the **ggplot2** code into a compact function, like so.
...
## Categorical variables {#sec-Categorical-variables}
...
It's a little absurd I'm covering this, here, because (a) I'm not a fan of the ANOVA framework and (b) McElreath didn't properly introduce the multilevel framework until @sec-Models-With-Memory. But we'll throw this in, anyway. For those of you who are new to the multilevel model, make a mental note of this section, back away slowly, and come back after you've worked through the later chapters in this text. For those of y'all who have some experience with the multilevel model, perhaps this will offer an application you hadn't considered.
...
The 'Group-Level Effects:' section in that output won't make much sense until we cover the material in @sec-Models-With-Memory. Here's how one might wrangle the posterior samples to make a coefficient plot similar to the one we made for the original `b5.9`.
...
In addition to waiting for @sec-Models-With-Memory in this ebook, you can learn more about the multilevel ANOVA approach in Chapters 19 and 24 of Kruschke's [-@kruschkeDoingBayesianData2015], in the corresponding chapters in my [-@kurzDoingBayesianDataAnalysis2026] ebook translation of the same, and in some of Gelman's peer-reviewed work [@gelmanAnalysisVarianceWhy2005; @gelmanPriorDistributionsVariance2006].
